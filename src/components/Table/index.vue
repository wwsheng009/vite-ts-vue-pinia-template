    ...
    >
      ...
      <template #bodyCell="{ column, text, index, record }">
        ...
        <template v-if="column.key === 'action'">
          <template
            v-for="(action, xindex) in getActions"
            :key="`${xindex}-${action.label}`"
          >
            <!-- 气泡确认框 -->
            <a-popconfirm
              v-if="action.enable"
            ...
            >
              <a @click.prevent="() => {}" :type="action.type">{{
                action.label
              }}</a>
            </a-popconfirm>
...
export default defineComponent({
  props: {
    ...
    url: {
      type: String /* 请求接口 promise */,
    },
    columns: {
      type: Array<Object> /* Table组件：columns 不包含操作列 */,
    },
    actions: {
      type: Object /* Table组件：操作列 */,
    },
    button: {
      type: Object /* Filter筛选列组件：交互按钮 */,
    },
    items: {
      type: Array<Object> /* Filter筛选列组件：包含的项 */,
    },
    model: {
      type: Object /* Filter筛选列组件：form model */,
    },
    resKey: {
      type: Object as PropType<Reskey>,
    },
    scroll: {
      type: Object,
    },
  ...
  setup(props) {
    ...
    const listData = computed(
      () =>
        (dataSource.value as unknown as Indexable)?.[
          props?.resKey?.list || 'list'
        ] || [],
    );
