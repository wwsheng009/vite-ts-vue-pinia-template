...
export function clearMenuItem(
  ...
  const filterHideMenus = menusData
    .map((item: RouteRecord | RouteRecordRaw) => {
      ...
      if (
        !finalItem.name ||
        finalItem.meta?.hideInMenu ||
        finalItem.meta?.hideMenu
      ) {
        return null;
      }
...
export const filterRoutes = (menusData: RouteRecordRaw[]): RouteRecordRaw[] => {
  ...
  return menusData
    .filter(({ path }) => !filterRoutes.includes(path))
    .filter((n) => !['/', '/login'].includes(n.path))
    .filter((n) => !n.path.startsWith('/redirect'));
};
